...
Use **exact** pins (no `^`). `@ai-sdk/react` declares `ai@7.0.79` as a **regular dependency** (not peer), so a mismatched direct `ai` produces two copies in `node_modules` and duplicated type identity at the `useChat`/transport boundary — the recurring strict-typing break. Matching the exact version lets npm dedupe to one copy.

## Hard gates (no-go if unmet)

- **Node ≥ 22** — all four packages declare `engines: { "node": ">=22" }`. CI typically runs 24.x; check local (`node -v`). Add `"engines": { "node": ">=22" }` to the app's `package.json` if you enforce it.
...
```bash
npm install --save-exact ai@7.0.79 @ai-sdk/react@4.0.82 @ai-sdk/mcp@2.0.37 @ai-sdk/openai@4.0.47
npm ls ai @ai-sdk/react   # must show ONE ai@7.0.79, deduped
```
